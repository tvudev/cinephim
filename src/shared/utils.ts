...
// export const embedMovie = (id: number): string => `${EMBED_URL}/${id}`;

export const embedMovie = (id: number): string => `${EMBED_VIDSRC}/movie?tmdb=${id}`;
...
export const embedTV = (id: number, season: number, episode: number): string =>
  `${EMBED_VIDSRC}/tv?tmdb=${id}&s=${season}&e=${episode}`;
